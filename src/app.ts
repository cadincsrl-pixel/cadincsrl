// La instancia de Hono con todas las rutas y middlewares, SIN escuchar en
// ningún puerto: eso lo hace index.ts.
//
// Está separada para que el backend pueda llamarse a sí mismo por HTTP interno
// con `app.request()`, sin puerto ni socket. Lo usa el asistente para crear un
// pedido dictado: entrando por la misma puerta que la pantalla hereda solo el
// permiso, el alcance por obra, la validación zod y la auditoría, en vez de
// duplicar esas cuatro cosas llamando al service por dentro.
...
import sueldosRoutes from './modules/sueldos/sueldos.routes.js'
...
app.route('/api/cuenta-cliente', cuentaClienteRoutes)
...
// Sueldos (2026-09-26): legajos, convenios, liquidaciones y recibos.
app.route('/api/sueldos', sueldosRoutes)
