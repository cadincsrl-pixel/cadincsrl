...
- medios: cada forma en que se paga, una vez cada una. forma = "cheque", "echeq", "transferencia", "deposito" o "efectivo". importe; numero (del cheque, o de operación/comprobante de la transferencia); banco (el del cheque, o el de origen de la transferencia); fecha_cobro (la fecha de pago del cheque, o la de la transferencia); librador y librador_cuit (sólo cheques: el TITULAR de la cuenta, la razón social impresa en el cheque, no la persona que firma); cuenta_destino (sólo transferencias o depósitos: el CBU, alias o banco de la cuenta de CADINC donde ENTRÓ, tal como figura). Nunca la cuenta de origen del cliente: en una orden de pago, la cuenta que aparece en «Fondos aplicados» o «Valores» suele ser la del cliente; si el documento no dice explícitamente que la cuenta es de CADINC (destino, beneficiario, CBU/alias de CADINC), devolvé null.
