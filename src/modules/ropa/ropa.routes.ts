...
// POST /api/ropa/entregas/tanda — entrega por obra: varios trabajadores juntos.
ropa.post('/entregas/tanda', zValidator('json', CreateEntregasTandaSchema), async (c) => {
  const data = await ropaService.createEntregasTanda(
    c.req.valid('json'),
    c.get('accessToken'),
    c.get('user').id,
  )
