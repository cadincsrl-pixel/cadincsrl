...
function handle<T>(fn: (c: any) => Promise<T>) {
  return async (c: any) => {
    try {
      ...
      if (err instanceof ChoferDocError) {
...
// GET /api/logistica/choferes/:id/documentos
docs.get(
  '/:id/documentos',
  requirePermiso('logistica', 'lectura'),
  handle(c => choferDocsService.listByChofer(Number(c.req.param('id')), c.get('accessToken'))),
)

// POST /api/logistica/choferes/:id/documentos/upload-url
docs.post(
  '/:id/documentos/upload-url',
  requirePermiso('logistica', 'creacion'),
  zValidator('json', UploadUrlSchema),
  handle(c => choferDocsService.generarUploadUrl(Number(c.req.param('id')), c.req.valid('json'))),
)

// POST /api/logistica/choferes/:id/documentos
docs.post(
  '/:id/documentos',
  requirePermiso('logistica', 'creacion'),
  zValidator('json', RegistrarSchema),
  handle(c => choferDocsService.registrar(
...
// PATCH /api/logistica/choferes/:id/documentos/:docId  → editar vence_el / obs
docs.patch(
  '/:id/documentos/:docId',
  requirePermiso('logistica', 'actualizacion'),
  zValidator('json', PatchSchema),
  handle(c => choferDocsService.actualizarMetadata(
    Number(c.req.param('id')),
...
// GET /api/logistica/choferes/:id/documentos/:docId/signed-url
docs.get(
  '/:id/documentos/:docId/signed-url',
  requirePermiso('logistica', 'lectura'),
  handle(c => choferDocsService.signedUrl(
    Number(c.req.param('id')),
    Number(c.req.param('docId')),
    c.get('accessToken'),
  )),
...
// DELETE /api/logistica/choferes/:id/documentos/:docId
docs.delete(
  '/:id/documentos/:docId',
  requirePermiso('logistica', 'eliminacion'),
  handle(c => choferDocsService.softDelete(
    Number(c.req.param('id')),
    Number(c.req.param('docId')),
    c.get('user').id,
    c.get('accessToken'),
